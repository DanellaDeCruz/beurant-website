// Island outline sourced from simplemaps.com (free for commercial/personal
// use, attribution appreciated — https://simplemaps.com/resources/svg-license).
// Pin coordinates are derived from that same SVG's calibration points (each
// tagged with its real lat/lon), fitting a linear lon→x / lat→y transform
// and applying it to each city's real-world coordinates.
const ISLAND_PATH =
...
const CITIES: CityPin[] = [
  { name: "Kandalama", x: 472, y: 507, dx: 14, dy: 4, anchor: "start" },
  { name: "Kurunegala", x: 406, y: 592, dx: -14, dy: -10, anchor: "end" },
  { name: "Kandy", x: 468, y: 637, dx: 14, dy: 4, anchor: "start" },
  { name: "Batticaloa", x: 712, y: 540, dx: -14, dy: 4, anchor: "end" },
  { name: "Wattala", x: 297, y: 708, dx: 14, dy: -10, anchor: "start" },
  { name: "Colombo", x: 290, y: 722, dx: 14, dy: 20, anchor: "start" },
  { name: "Bentota", x: 321, y: 839, dx: 14, dy: 4, anchor: "start" },
  { name: "Galle", x: 373, y: 925, dx: 0, dy: -16, anchor: "middle" },
  { name: "Matara", x: 445, y: 950, dx: 14, dy: -12, anchor: "start" },
];

export default function SriLankaMap() {
  return (
    <svg
      viewBox="242 45 516 910"
      className="h-full w-full"
      role="img"
      aria-label="Map of Sri Lanka with pins marking Kandalama, Kurunegala, Batticaloa, Bentota, Galle, Matara, Kandy, Colombo and Wattala"
    >
      <path
        d={ISLAND_PATH}
        fill="var(--surface-2)"
      ...
      {CITIES.map((city) => (
        <g key={city.name}>
          <circle
            ...
            fillOpacity="0.25"
          />
          <circle cx={city.x} cy={city.y} r="4.5" fill="var(--accent)" />
          <text
            x={city.x + city.dx}
            y={city.y + city.dy}
